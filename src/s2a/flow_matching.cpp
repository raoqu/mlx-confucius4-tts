...
Tensor ConditionalCFM::solve_euler(Tensor x, const Tensor& t_span,
  ...
  for (int step = 1; step < n; ++step) {
    Tensor t_arr = mx::full(mx::Shape{B}, t);  // (B,) current time
    Tensor dphi_dt = estimator_.forward(x, mask, mu, t_arr, spks, prompt_x);
    if (cfg_rate > 0) {
      // Classifier-free guidance: combine conditional and unconditional fields.
      Tensor uncond_v =
          estimator_.forward(x, mask, zero_mu, t_arr, zero_spks, zero_prompt);
      dphi_dt = mx::subtract(mx::multiply(mx::array(1.0f + cfg_rate), dphi_dt),
                             mx::multiply(mx::array(cfg_rate), uncond_v));
    }

    x = mx::add(x, mx::multiply(mx::array(dt), dphi_dt));
    t += dt;
