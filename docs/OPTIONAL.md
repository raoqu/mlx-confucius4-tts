...
### A1. S2A 换更高阶 ODE 求解器（最干净的免费提速）
当前是一阶 Euler @ 25 步。flow-matching 路径近似直线，用
**DPM-Solver++ / Heun / midpoint**（多步复用速度场）通常能在 ~10–12 步达到 Euler-25
的质量 → S2A **~2×**。
...
### B1. T2S 投机 / 多 token 解码（单点最高杠杆）
T2S 占 54%，AR 是"每次一个 token"的死结。打破它的标准武器：
- **Medusa 式多预测头**：在同一 hidden 上加几个轻量头预测 t+2/t+3，主模型一次前向
  验证多个。只需训练这几个小头（很便宜，不动主干）→ T2S **~2–2.5×**。
- **投机解码**：小草稿模型 / n-gram 草稿（音频 semantic token 流有局部重复性）猜 k 个，
  主模型批量验证。
